...
class IncrementalSampleScorer:
    def __init__(self):
        self.scored_file = "data/universal_samples_scored.txt"
        self.scoring_log = "data/training/scoring_log.json"
    ...
    def prepare_incremental_training_data(self, samples):
        ...
        # 保存增量ML训练数据
        if ml_annotations:
            ml_file = "data/training/incremental_ml_annotations.json"
        ...
        # 保存增量生成模型训练数据
        if generation_data:
            gen_file = "data/training/incremental_generation_data.jsonl"
