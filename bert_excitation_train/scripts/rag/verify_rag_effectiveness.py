...
def analyze_generated_content():
    ...
    # 分析RAG生成的内容
    rag_dir = "bert_excitation_train/data/generated/暗河噬城"
    rag_scores = []
...
def compare_rag_effectiveness():
    ...
    # 检查RAG样本库使用情况
    print(f"\nRAG样本库使用情况:")
    if os.path.exists('bert_excitation_train/data/universal_samples_vectors.npy'):
        print(f"  ✅ 样本库已初始化")
        
        # 检查样本库内容
        if os.path.exists('bert_excitation_train/data/universal_samples_data.json'):
            with open('bert_excitation_train/data/universal_samples_data.json', 'r', encoding='utf-8') as f:
                data = json.load(f)
