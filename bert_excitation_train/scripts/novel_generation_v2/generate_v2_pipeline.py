...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Unified V2 pipeline: clusters -> outline -> chapters "
    ...
    scripts_dir = repo_root / "bert_excitation_train" / "scripts" / "novel_generation_v2"
    ...
    if not args.skip_chapters:
        if not args.skip_neo4j_setup:
            ...
            bootstrap_cmd = [
                args.python,
                "-m",
                "bert_excitation_train.scripts.knowledge_graph.bootstrap_neo4j",
            ]
            run_step(
            ...
            run_step(
                ...
                [
                    args.python,
                    "-m",
                    "bert_excitation_train.scripts.knowledge_graph.build_plot_clusters",
                    "--clusters-config",
