...
def interactive_annotation():
    ...
    if annotations:
        # 保存标注
        output_file = f"data/training/annotations_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
...
def batch_annotation():
    ...
    if all_annotations:
        # 保存所有标注
        output_file = f"data/training/batch_annotations_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
...
def train_model():
    ...
    if scorer.train_model(all_annotations):
        ...
        # 保存合并的标注和模型
        merged_file = f"data/training/merged_annotations_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
