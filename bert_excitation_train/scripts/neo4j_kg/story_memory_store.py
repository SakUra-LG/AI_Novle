...
def replace_chapter_memory(driver: Driver, memory: Dict[str, Any]) -> None:
        ...
        def _replace(tx) -> None:
            ...
            for index, item in enumerate(events):
                ...
                tx.run(
                    ...
                    story_id=story_id,
                    chapter=chapter,
                    event_order=int(item.get("event_index", index)),
            ...
            for index, item in enumerate(all_facts):
                ...
                tx.run(
                    ...
                    CREATE (f:StoryFact {id:$id, story_id:$story_id, source_chapter:$chapter, subject:$subject,
                      predicate:$predicate, object:$object, old_value:$old_value,
                    ...
                    MERGE (ch)-[:ASSERTS]->(f)
                    """,
                    id=fact_id, story_id=story_id, chapter=chapter, subject=subject, predicate=predicate, object=obj,
            ...
            _rebuild_current_character_views(tx)
            _rebuild_character_chapter_bounds(tx)
            _rebuild_current_relationship_views(tx)
            _rebuild_current_thread_views(tx)
            _rebuild_temporal_edges(tx)

        session.execute_write(_replace)


def _rebuild_current_character_views(tx) -> None:
    tx.run(
        """
        MATCH (c:Character)
        OPTIONAL MATCH (f:StoryFact)-[:ABOUT]->(c)
        WHERE coalesce(f.timeline, 'current') IN ['current', 'unknown']
...
def _rebuild_current_relationship_views(tx) -> None:
    tx.run("MATCH ()-[r:CURRENT_RELATION]->() DELETE r")
    tx.run(
        """
        MATCH (r:RelationFact)-[:FROM_CHARACTER]->(a:Character)
        MATCH (r)-[:TO_CHARACTER]->(b:Character)
        WHERE coalesce(r.timeline, 'current') IN ['current', 'unknown']
...
def _rebuild_character_chapter_bounds(tx) -> None:
    tx.run(
        """
        MATCH (c:Character)
...
def _rebuild_current_thread_views(tx) -> None:
    tx.run(
        """
        MATCH (t:PlotThread)
...
def _rebuild_temporal_edges(tx) -> None:
    tx.run("MATCH ()-[r:NEXT_CHAPTER|NEXT_EVENT]->() DELETE r")
    tx.run(
        """
        MATCH (a:StoryChapter), (b:StoryChapter)
        WHERE a.story_id = b.story_id AND b.number = a.number + 1
    ...
    tx.run(
        """
        MATCH (a:StoryEvent), (b:StoryEvent)
        WHERE a.story_id = b.story_id AND b.source_chapter = a.source_chapter + 1
          AND NOT EXISTS {
            MATCH (later:StoryEvent)
            WHERE later.source_chapter=a.source_chapter AND later.event_order > a.event_order
          }
          AND NOT EXISTS {
            MATCH (earlier:StoryEvent)
            WHERE earlier.source_chapter=b.source_chapter AND earlier.event_order < b.event_order
          }
        MERGE (a)-[:NEXT_EVENT]->(b)
        """
    )
    ...
    with driver.session() as session:
        def _delete(tx) -> None:
            tx.run(
                ...
                DETACH DELETE n
                """,
                chapters=chapters, story_id=story_id,
            )
            tx.run(
                "MATCH (ch:StoryChapter) WHERE ch.story_id=$story_id AND ch.number IN $chapters DETACH DELETE ch",
                chapters=chapters, story_id=story_id,
            )
            _delete_orphan_aggregates(tx, story_id)
            _rebuild_current_character_views(tx)
